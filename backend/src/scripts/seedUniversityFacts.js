...
async function seedFacts() {
    ...
    for (const fact of facts) {
        ...
        const embedding = await (0, embeddingService_1.generateEmbedding)(fact);
        vectors.push({
            id: id,
            values: embedding,
            metadata: {
    ...
    await (0, pineconeService_1.upsertKnowledgeVectors)(vectors);
    console.log("Successfully seeded university facts to Pinecone.");
    process.exit(0);
}
seedFacts().catch(console.error);
//# sourceMappingURL=seedUniversityFacts.js.map