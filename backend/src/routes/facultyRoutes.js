...
router.get('/', async (req, res) => {
    try {
        const faculty = await Faculty_1.default.find().sort({ firstName: 1 });
        res.json(faculty);
    }
    catch (err) {
...
router.post('/search', async (req, res) => {
    try {
        ...
        const allFaculty = await Faculty_1.default.find().select('_id firstName lastName department designation researchInterests bio');
...
Based on the researchInterests, department, and bio, return ONLY a valid JSON array of the "_id" strings of the top matching faculty members. Do not include any other text, markdown formatting, or explanation. Just the JSON array. Example: ["60d5ecb8b392d700153c3d5a", "60d5ecb8b392d700153c3d5b"]`;
        ...
        const matchedFaculty = await Faculty_1.default.find({ _id: { $in: resultIds } });
...
router.get('/:id', async (req, res) => {
    try {
        const faculty = await Faculty_1.default.findById(req.params.id);
        if (!faculty)
